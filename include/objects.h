...
struct git_obj_blob *create_blob_from_path(const char *filepath);

// Creates blob file in objects folder, if it does not already exist.
// @return 0 if successful, -1 otherwise.
int write_blob_to_disk(const git_repo *, const struct git_obj_blob *);

// Creates blob struct from blob file. 
// @return pointer to blob or NULL if could not read file.
struct git_obj_blob *create_blob_from_disk(const git_repo * repo, obj_hash);
...
struct git_obj_tree *create_tree_from_path(const char *folderpath);
...
// @return 0 if successful, -1 otherwise.
int write_tree_to_disk(const git_repo *repo, const struct git_obj_tree *);

// Creates tree struct from tree file.
// @return pointer to tree or NULL if could not read file.
struct git_obj_tree *create_tree_from_disk(const git_repo *repo, obj_hash hash);
